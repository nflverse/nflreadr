test_that("cleaning team abbreviations", {
  ...
  x <- c("PIE","LAR","PIT","CRD", "OAK", "CLV", "PHO", "Niners", "49ers", "San Diego Chargers", "Oakland Raiders", "St Louis Rams")
  ...
  expect_equal(new_abbr,      c("PIE", "LA","PIT","ARI","LV","CLE", "ARI", "SF", "SF", "LAC", "LV", "LA"))
  expect_equal(new_abbr_drop, c(NA, "LA","PIT","ARI","LV","CLE", "ARI", "SF", "SF", "LAC", "LV", "LA"))
  expect_equal(old_abbr,      c("PIE", "LA","PIT","ARI","OAK","CLE", "ARI", "SF", "SF", "SD", "OAK", "STL"))
  options(o)
})

test_that("cleaning player names", {

  p <- c("Trubisky,      Mitch", "Atwell, Chatarius", "Elliott, Zeke", "Elijah Moore", "A.J. Green", "Odell Beckham Jr.   ", "Audric Estimé")

  lower <- clean_player_names(p,lowercase = TRUE, use_name_database = TRUE, convert_lastfirst = TRUE)

  expect_equal(
    lower,
    c("mitchell trubisky", "tutu atwell","ezekiel elliott", "elijah moore", "aj green", "odell beckham", "audric estime"))
